#!/bin/sh

PORT_APISIX_API="9180"
PORT_APISIX_ENTRY="9080"
PORT_APISIX_PROMETHEUS="9091"
PORT_APISIX_CONTROL="9092"
PORT_APISIX_DASHBOARD="9000"
PORT_ETCD="2379"
PORT_REDIS="6379"
PORT_POSTGRES="5432"
PORT_KEYCLOAK="8080"
PORT_KEYCLOAK_INTERNAL="8080"
PORT_PORTAL="9081"
PORT_PORTAL_INTERNAL="80"
PORT_TRAMPOLINE="9082"
PORT_TRAMPOLINE_INTERNAL="8080"
PORT_CHATGPT_API="5000"
PORT_CHATGPT_WS="8765"
PORT_FAUXPILOT="5001"
PORT_FAUXPILOT_INTERNAL="5000"
PORT_PROMETHEUS="9090"
PORT_GRAFANA="3000"
PORT_ES="9200"

ZGSM_BACKEND="172.31.86.242"
APISIX_ADDR="127.0.0.1:${PORT_APISIX_API}"
APIKEY_APISIX_ADMIN="edd1c9f034335f136f87ad84b625c8f1"
APIKEY_APISIX_VIEWER="4054f730f8e344346cd3f287985e76a2"

AUTH="X-API-KEY: ${APIKEY_APISIX_ADMIN}"
TYPE="Content-Type: application/json"

PASSWORD_APISIX_DASHBOARD="sf2025~SHENMA"
PASSWORD_ETCD="sf2025~SHENMA"
PASSWORD_REDIS="sf2025~SHENMA"
PASSWORD_POSTGRES="sf2025~SHENMA"
PASSWORD_ES="sf2025~SHENMA"
PASSWORD_KEYCLOAK="sf2025~SHENMA"
PASSWORD_ELASTIC="4c6y4g6Z09T2w33pYRNKE3LG"

# ES
ENROLLMENT_TOKEN="4c6y4g6Z09T2w33pYRNKE3LG"

KEYCLOAK_ADDR="http://127.0.0.1:${PORT_KEYCLOAK}"
KEYCLOAK_INTERNAL_ADDR="http://keycloak:${PORT_KEYCLOAK_INTERNAL}"
KEYCLOAK_CLIENT_ID="vscode"
KEYCLOAK_CLIENT_SECRET="jFWyVy9wUKKSkX55TDBt2SuQWl7fDM1l"
KEYCLOAK_REALM="gw"
KEYCLOAK_USERNAME="zgsm"
KEYCLOAK_PASSWORD="123"

 # root key
ONE_API_INITIAL_ROOT_KEY="966c3157fe65461dbc731cd540b6cd5d"
# User authentication token, should not exceed 32 characters
ONE_API_INITIAL_ROOT_ACCESS_TOKEN="e9e3-4cd3-abfc-36707f44ee1e"
ONE_API_HOST=one-api
ONE_API_PORT=30000

# 对话模型
CHAT_MODEL="deepseek-chat"
# 补全模型
OPENAI_MODEL_HOST="http://http://localhost:8002/v1/completions"
OPENAI_MODEL="deepseek-code"
OPENAI_MODEL_API_KEY="966c3157fe65461dbc731cd540b6cd5d"

# OIDC
##cloak
OIDC_BASE_URL="/realms/$KEYCLOAK_REALM"
OIDC_HOST="keycloak"
OIDC_PORT=${PORT_KEYCLOAK_INTERNAL}
OIDC_CLIENT_ID=${KEYCLOAK_CLIENT_ID}
OIDC_CLIENT_SECRET=${KEYCLOAK_CLIENT_SECRET}


# aigateway
AIGATEWAY_HOST="${AIGATEWAY_HOST}"
AIGATEWAY_PORT="${AIGATEWAY_PORT}"

# quota-manager
QUOTA_MANAGER_HOST="${QUOTA_MANAGER_HOST}"
QUOTA_MANAGER_PORT="${QUOTA_MANAGER_PORT}"

# casdoor
CASDOOR_HOST="${CASDOOR_HOST}"
CASDOOR_PORT="${CASDOOR_PORT}"
CASDOOR_CLIENT_ID="${CASDOOR_CLIENT_ID}"
CASDOOR_CLIENT_SECRET="${CASDOOR_CLIENT_SECRET}"

# oidc-auth
OIDC_AUTH_HOST="${OIDC_AUTH_HOST}"
OIDC_AUTH_PORT="${OIDC_AUTH_PORT}"

# credit-manager
CREDIT_MANAGER_HOST="${CREDIT_MANAGER_HOST}"
CREDIT_MANAGER_PORT="${CREDIT_MANAGER_PORT}"