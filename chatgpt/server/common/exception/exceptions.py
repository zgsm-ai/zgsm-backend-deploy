...
class NoResourceError(CustomException):
    ...
    def __init__(self, resource_id=None, label=None):
        if resource_id:
            if label:
                self.msg = f"Could not find {label} data with ID {resource_id}"
            else:
                self.msg = f"Could not find resource data with ID {resource_id}"
...
class AuthFailError(CustomException):
    msg = 'You do not have permission for this operation, please contact a person with operation permission to add permission'
    code = 401
...
class NodeAuthFailError(CustomException):
    msg = 'You do not have permission for this operation, please contact the maintainer to add permission'
    code = 401
    error_code = ERROR_CODE.AUTH.FORBIDDEN
    send_to_sentry = False
...
class PermissionFailError(CustomException):
    msg = 'You do not have permission for this operation, please contact the administrator to add permission'
    code = 403
    error_code = ERROR_CODE.AUTH.FORBIDDEN
    send_to_sentry = False
...
class RequireParamsMissingError(CustomException):
    msg = "Required parameters missing exception"
    code = 500
...
class ResHeadersParameterError(CustomException):
    msg = "Response headers parameter exception"
    code = 400
...
class ManualCaseError(CustomException):
    msg = "The test case steps are too simple, please improve them before generating"
    code = 400
