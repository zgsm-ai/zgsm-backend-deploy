...
# Cacheable: used to define the cache
# CacheEvict: used to clear the cache
# CachePut: used to update the cache
# @Caching

# If there is a cache, return the cache. If not, execute and then cache
# Pass value with key, key=value
# Index passes the subscript of the parameter that needs to be cached
def cache_able(prefix, key=None, index=None, expire=get_config().get("default_cache_expire_seconds")):
    def wrapper(func):
        def inner(*args, **kwargs):
            ...
            if cache_value:  # Prevent saving to the cache even if no data is found
                if isinstance(cache_value, peewee.ModelSelect):  # Arrays and single objects are saved separately
...
# Delete all caches with the specified prefix
...
def push_cache(prefix, key=None, index=None):
    def wrapper(func):
        def inner(*args, **kwargs):
            ...
            cache_value = func(*args, **kwargs)
            if cache_value:
                if isinstance(cache_value, peewee.Model):  # Arrays and single objects are saved separately
