...
def init_es():
    ...
    # Set the log level, because during development, the query results will be printed, which is annoying
...
def calc_rid(*args) -> str:
    ...
    # Remove the '=' used for padding at the end
...
class BaseESService:
    ...
    def insert(self, obj_dict: dict, id: str = None):
        ...
        try:
            ...
            self.logger.error(f"Add es:{self.index} index exception:{str(err)},obj:{obj_dict}")
            raise EsIndexError()

    def bulk_insert(self, objects):
        """
        Batch insert json.dumps(objects, self=CustomJSONEncoder)
        :param objects:
        :return:
        """
        self.logger.info(f"Batch insert {len(objects)} items")
        insert_s = list()
    ...
    def update_by_id(self, id, update_data):
        try:
            ...
            self.logger.error(f"Modify es:{self.index} id :{id}, update_data:{update_data} failed：{str(e)}")
            return None
