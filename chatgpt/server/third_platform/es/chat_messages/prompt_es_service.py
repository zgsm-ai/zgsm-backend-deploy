...
class PromptESService(BaseESService):
    ...
    def insert_prompt(self, data, response_content='', usage=None):
        """
        Insert prompt request data, exceptions during insertion do not affect the main process execution
        :param data: Interface request parameter information
        :param response_content: Interface request content return
        :param usage: Tokens consumed by the interface
        """
        try:
            rid = data.get("id")
            if rid is None:
            ...
            self.logger.error(f"Failed to insert prompt data into es, error log: {str(err)}")
